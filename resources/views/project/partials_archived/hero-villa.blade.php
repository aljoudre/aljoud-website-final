<!-- Villa Layout Style (Minimal header with video/stats) -->
<section class="relative {{ $hasHeaderImage && $headerImage ? 'h-screen' : 'py-20' }} overflow-hidden bg-[#f8f7f3]">
    ...
    @if($project->enable_3d_tour && $project->tour_360_url)
        <div class="relative z-10 h-full flex items-center justify-center px-4 py-12">
            <div class="max-w-4xl mx-auto text-center text-white">
                <h1 class="text-4xl md:text-7xl font-bold mb-4">{{ $project->translate('name') }}</h1>
                <h2 class="text-2xl md:text-4xl font-light mb-8">{{ $project->translate('subtitle') }}</h2>
    ...
    @else
        <div class="{{ $hasHeaderImage && $headerImage ? 'relative z-10 h-full flex items-center justify-center' : '' }} px-4 py-12">
